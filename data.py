...
for ticker_symbol in tickers:
    ...
    for i in range(len(history)):
        ...
        if ratio >= 1.5 and change_percent >= 2.0:
            print(f"{ticker_symbol} — {date} — Volume {ratio:.2f}x — Change {change_percent:.2f}% — STRONG SIGNAL: Volume spike + price increase")
        elif ratio >= 1.5 and change_percent <= -2.0:
            print(f"{ticker_symbol} — {date} — Volume {ratio:.2f}x — Change {change_percent:.2f}% — STRONG SIGNAL: Volume spike + price decrease")