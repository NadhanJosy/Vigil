...
def compute_mtf(history):
    def assess_trend(closes):
        ...
        if slope > 3 and above_mean:
            ...
            return "DOWN"
        return "NEUTRAL"
    df_weekly = history.resample("W-MON").agg({
        "Open": "first", "High": "max", "Low": "min",
        "Close": "last", "Volume": "sum"
    }).dropna()
    weekly = assess_trend(df_weekly["Close"]) if len(df_weekly) >= 4 else "NEUTRAL"
    daily  = assess_trend(history["Close"].iloc[-20:]) if len(history) >= 20 else "NEUTRAL"
...
def run_detection():
    ...
    for ticker_symbol in tickers:
        try:
            if ticker_symbol not in all_data.columns.get_level_values(0):
                continue
            
            history = all_data[ticker_symbol].copy().dropna()
            if len(history) < 20:
                continue
            ...
            if ratio >= 1.5 and change_percent >= 2.0:
                ...
                # CRITICAL: Edge score calculation was missing a check for recent accumulation conviction
                accum_conv = None
                if has_recent_accum:
                    recent_alert = get_recent_alert_for_ticker(ticker_symbol, "ACCUMULATION_DETECTED")
                    accum_conv = recent_alert.get('accum_conviction') if recent_alert else None
                
                edge    = compute_edge_score(combo, mtf_alignment, trap_conv, ratio, days_in, accum_conv)
