...
class AlertRouter:
    ...
    async def dispatch(self, alert: dict) -> list:
        results = []
        for name, ch in self.channels.items():
            results.append(await self._dispatch_one(name, ch, alert))
        return results

    async def _dispatch_one(self, name: str, channel, alert: dict) -> DeliveryResult:
        last_error = None
