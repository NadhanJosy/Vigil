...
class RegimeEngine:
    ...
    def classify(self, spy_history: pd.DataFrame) -> RegimeResult:
        ...
        RegimeResult
        """
        if spy_history is None or spy_history.empty or len(spy_history) < 50:
            logger.warning("Insufficient SPY history for regime classification. Falling back to SIDEWAYS.")
            return RegimeResult("SIDEWAYS", 0.5, 1.0, 0.0, 50.0, factors={"error": "insufficient_data"})
        ...
        # Ensure scores exists before getting
        confidence = scores.get(regime, 0.0) if scores else 0.0
